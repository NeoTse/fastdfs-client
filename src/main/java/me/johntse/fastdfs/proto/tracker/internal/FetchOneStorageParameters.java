...
/**
 * 根据文件路径选择一个Storage时所需参数.
 *
 * @author johntse
 * @since 0.1.0
 */
public class FetchOneStorageParameters extends AbstractRequestParameters {
    /**
     * 文件所在分组名称.
     */
    ...
    public FetchOneStorageParameters() {
        super(ProtocolConstants.TRACKER_PROTO_CMD_SERVICE_QUERY_FETCH_ONE);
    }
     ...
     * @param filePath  文件路径
     */
    public FetchOneStorageParameters(String groupName, String filePath) {
        this();

        if (groupName == null || groupName.isEmpty()) {
            throw new IllegalArgumentException("FetchOneStorageParameters: group name can't be empty.");
        }

        if (filePath == null || filePath.isEmpty()) {
            throw new IllegalArgumentException("FetchOneStorageParameters: file path can't be empty.");
        }
